...
extern __IO uint8_t Endp1_Complete,Endp2_Complete;
...
BOOL hid_send_mouse(int8u key,int8s x,int8s y)
{
	...
	USB_SIL_Write(EP2_IN, mouse_buffer, 4);
...
BOOL hid_send_keyboard(int8u* keys)
{
	...
	for(i=0;i<100;i++)
	{
		if(bDeviceState == CONFIGURED && Endp1_Complete==1)
			break ;
		delay_us(100);
	}
	if(bDeviceState != CONFIGURED||Endp1_Complete==0)
		return FALSE;
	Endp1_Complete=0;
	/* Copy mouse position info in ENDP1 Tx Packet Memory Area*/
	USB_SIL_Write(EP1_IN, key_buff, 8);
	/* Enable endpoint for transmission */
	SetEPTxValid(ENDP1);
	return TRUE;
}

static int8u hid_buff[65];
BOOL hid_send_data(int8u* data,int8u length)
{
	int i;
	if(length>64)
		return FALSE;
	...
	if(bDeviceState != CONFIGURED||Endp1_Complete==0)
		return FALSE;
	memset(hid_buff,0,65);
	hid_buff[0]=1;
	memcpy(hid_buff+1,data,length);
	Endp1_Complete=0;
	/* Copy mouse position info in ENDP1 Tx Packet Memory Area*/
	USB_SIL_Write(EP1_IN, hid_buff, 65);
	/* Enable endpoint for transmission */
	SetEPTxValid(ENDP1);
	return TRUE;
}
