...
ONE_DESCRIPTOR Custom_Report_Descriptor=
{
	  (u8 *)CustomHID_ReportDescriptor,									//
    Custom_ReportDescriptor_Size										//
};

ONE_DESCRIPTOR Custom_Hid_Descriptor =									//
{																	//
	(u8*)Joystick_ConfigDescriptor + Custom_OFF_HID_DESC,				//
...
void Joystick_Reset(void)
{
  ...
  SetEPTxCount(ENDP1, CUSTOMHID_MSG_LENGTH);
  SetEPRxCount(ENDP1, CUSTOMHID_MSG_LENGTH);
      ...
      && (pInformation->USBwIndex0 < 3))
  {
    ...
    if (pInformation->USBwValue1 == REPORT_DESCRIPTOR)
    {
			if(pInformation->USBwIndex0 == 0)
				CopyRoutine = Custom_GetReportDescriptor;
    ...
    {
    	if (pInformation->USBwIndex0 == 0)
				CopyRoutine = Custom_GetHidDescriptor;
			else if(pInformation->USBwIndex0 == 1)
...
//}
u8 *Custom_GetReportDescriptor(u16 Length)
{
	return Standard_GetDescriptorData(Length, &Custom_Report_Descriptor);
}
...
* Return         : The address of the configuration descriptor.
*******************************************************************************/
u8 *Custom_GetHidDescriptor(u16 Length)
{
	return Standard_GetDescriptorData(Length, &Custom_Hid_Descriptor);
}
